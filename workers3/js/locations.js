...
function show_locations(){
	...
	$.each(available_locations, function(location_id, location_info){
		var can_unlock = true;
		$.each(location_info['cost'], function(item_id, item_amount){
			if(gamedata['produced'][item_id] == undefined)
			{
				can_unlock = false;
			}
		});
		var current_level = 0;
		if(location_info['needs_locations'][1] != undefined)
		{
			
			$.each(location_info['needs_locations'][1], function(needed_id, needed_level){
		...
		/*if(can_unlock == true && gamedata['owned_locations'][location_id] == undefined)
		{
			var parsed_location = parse_location(location_id, false);
			unowned_locations += parsed_location;
		}
	...
	if(current_location_page > max_inventory_page && max_inventory_page > 0){
...
function parse_location(location_id, owned){
	...
	if(location_collapsed == '')
	{
		parsed_location += 		'<div class="location_name" onclick="current_location=\'' + location_id + '\';show_content(\'location_details\')">' + capitalizeFirstLetter(current_location['name']) + ' ' + level_name + '</div>';
	}
	else
	{
		parsed_location += 		'<div class="location_name" onclick="show_content(\'locations\')">' + capitalizeFirstLetter(current_location['name']) + ' ' + level_name + '</div>';
	}
	if(owned == false)
	{
		...
		if(current_location['bonus_items'] != undefined)
		{
			...
			$.each(current_location['bonus_items'], function(bonus_id, min_level){
				if(min_level <= location_level + 1)
				{
					...
					parsed_location += 	'<div class="location_bonus_item ' + next_unlock + '" style="background-image:url(images/' + available_items[bonus_id]['image'] + ')"><div class="location_bonus_amount">' + bonus_amount_string + '</div></div>';
				}
			});
		}
		if(current_location['unlock_workers'] != undefined)
		{
			...
			$.each(current_location['unlock_workers'], function(bonus_id, min_level){
				if(min_level <= location_level + 1)
				{
					...
					parsed_location += 	'<div class="location_bonus_item ' + next_unlock + '" style="background-image:url(images/' + available_workers[bonus_id]['image'] + ')"></div>';
				}
			});
		}
		if(current_location['unlock_customers'] != undefined)
		{
			...
			$.each(current_location['unlock_customers'], function(bonus_id, min_level){
				if(min_level <= location_level + 1)
				{
					...
					parsed_location += 	'<div class="location_bonus_item ' + next_unlock + '" style="background-image:url(images/' + all_customers[bonus_id]['image'] + ')"></div>';
				}
			});
		}

		if(current_location['upgrade_costs'][location_level + 1] != undefined || location_level == 0 || true)
		{
			if(have_needed_locations == true)
			{
				...
				if(location_level >= 0)
				{
					var current_cost_list = true_copyobject(current_location['cost']);
					var upgrade_cost_factor = calculate_location_upgrade_cost(location_level + current_location['tier']);
					$.each(current_location['upgrade_costs'], function(upgrade_cost_level, upgrade_cost_items){
						if(upgrade_cost_level <= location_level + 1)
						{
							$.each(upgrade_cost_items, function(upgrade_cost_item_id, upgrade_cost_item_level){
								current_cost_list[upgrade_cost_item_id] = upgrade_cost_item_level;
							});
						}
					});
					...
					var total_upgrade_cost_value = 0;
					$.each(current_cost_list, function(item_id, item_amount){
						total_upgrade_cost_value += available_items[item_id]['value'] * item_amount;
					});
					upgrade_cost_factor = Math.ceil(upgrade_cost_factor / total_upgrade_cost_value);
					$.each(current_cost_list, function(item_id, item_amount){
		...
		parsed_location +=	'<div class="cost_progress_bar" style="width:' + progress_percent + '%;background-color:' + get_percent_color(total_owned_cost, total_cost) + '"></div>';
