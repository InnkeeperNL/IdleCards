...
function process_ability(unit_id, current_ability, level, origin_id, any_effect_fired, current_proc, check_death, fixed_proc_chance){
	...
	if(battle_info['combat_units'][unit_id] != undefined)
	{
		...
		if(current_ability['origin_subtypes'] != undefined && battle_info.combat_units[origin_id] != undefined)
		{
			//console.log(match_array_values(current_ability['origin_subtypes'], battle_info.combat_units[origin_id]['subtypes']));
			if(match_array_values(current_ability['origin_subtypes'], battle_info.combat_units[origin_id]['subtypes']) == false)
			{
				ability_can_fire = false;
			}
		}
		...
		if(current_ability['min_ally_structure_units'] != undefined && count_ally_units(battle_info['combat_units'][unit_id]['side'], 'structure') < current_ability['min_ally_structure_units'])
		{
			ability_can_fire = false;
		}
	...
	if(battle_info.combat_units[origin_id] != undefined)
	{
		...
		if(battle_info.combat_units[origin_id] != undefined)
		{
			...
			if(battle_info.combat_units[target_id] != undefined)
			{
				...
				if(effect_avoided == false && match_array_values(effect['subtypes'], 'projectile') && battle_info.combat_units[target_id]['armor'] != undefined && battle_info.combat_units[target_id]['armor'] > 0 && Math.random() < battle_info.combat_units[target_id]['armor'] / armor_projectile_reflection)
				{
					effect_avoided = true;
					show_avoided = false;
			        latest_result = 0;
...
function calculate_effect(effect, target_id, origin_id, level){
	...
	if(battle_info.combat_units[origin_id] != undefined)
	{
		...
		if(calculated_amount == 'ally_structure_count')
		{
			if(battle_info.combat_units[origin_id]['side'] == 2)
			{
				calculated_amount = count_ally_units(2, 'structure');
			}
			else
			{
