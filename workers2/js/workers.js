...
function parse_worker_bar(worker_id){
	...
	if(gamedata['workers'][worker_id] != undefined)
	{
		...
		parsed_worker_bar += 	'<div class="worker_bar_image" style="background-image:url(images/' + worker_info['image'] + ')" onclick="current_worker_details=' + worker_id + ';show_worker_upgrades=true;show_content(\'worker_details\')"><div class="worker_level_string">' + worker_level_string + '</div></div>';
		...
		if(available_actions[worker['action_id']] != undefined)
		{
			...
			if(available_actions[worker['action_id']]['image'] != undefined)
			{
				...
				if(available_actions[action_id] != undefined)
				{
					...
					if(can_show_action == true)
					{
						...
						switch(current_action_info['type']){
							case 'resource':
								selected_time_amount = (worker['endurance_level'] * worker_info['base_endurance']);
								var main_item = '';
								var gained_amount = 0;
							...
							case 'crafting':
								...
								$.each(current_action_info['cost'], function(cost_id, cost_amount){
									...
									parsed_worker += 	'<div class="worker_action_image cost_image have_enough_' + have_enough + '" style="background-image:url(images/' + available_items[cost_id]['image'] + ')"><div class="action_item_owned">' + owned_amount + '</div><div class="action_item_earned">-' + (cost_amount * selected_time_amount) + '</div></div>';
								});
								$.each(current_action_info['loot'], function(cost_id, cost_amount){
									...
									if(gamedata['produced'][cost_id] != undefined)
									{
										parsed_worker += 	'<div class="worker_action_image" style="background-image:url(images/' + available_items[cost_id]['image'] + ')"><div class="action_item_owned">' + owned_amount + '</div><div class="action_item_earned">+' + result_amount + '</div></div>';
									}
									else
									{
...
function calculate_worker_upgrade_cost(current_level){
	...
	if(current_level > 0)
	{
		worker_level_cost = current_level * level_cost;
	}
...
function show_action_details(){
	...
	if(selected_time_amount >= (worker['endurance_level'] * worker_info['base_endurance']))
	{
		selected_time_amount = (worker['endurance_level'] * worker_info['base_endurance']);
...
function show_claim_action(){
	...
	if(worker['done_time'] <= now)
	{
		...
		else
		{
			...
			var action_amount_effect = (0 + (worker['action_amount'] / 1));
			...
			ally['side'] 	= 'ally';
			ally['power']	= worker_info['stats']['power'] * worker['endurance_level'];
			ally['max_hp']	= worker_info['stats']['max_hp'] * worker['endurance_level'];
